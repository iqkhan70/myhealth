...
# Pass DEPLOYMENT_MODE into the remote shell
ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "$DROPLET_USER@$DROPLET_IP" "DEPLOYMENT_MODE=$DEPLOYMENT_MODE bash -s" << 'ENDSSH'
    set -e
...
if [ "$DEPLOYMENT_MODE" = "local-db" ]; then
    ...
    ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "$DROPLET_USER@$DROPLET_IP" << ENDSSH
        ...
        if [ \$USER_CREATE_EXIT -ne 0 ] || mysql -u root -e "SELECT User FROM mysql.user WHERE User='$DB_USER' AND Host='localhost';" 2>&1 | grep -v "Warning" | grep -v "Enter password" | grep -q "$DB_USER"; then
            ...
            if [ "\$SQL_METHOD" = "DOTNET_EF" ]; then
                ...
                if [ -f SM_MentalHealthApp.Server.csproj ]; then
                    ...
                    if dotnet build SM_MentalHealthApp.Server.csproj 2>&1 | tail -20; then
                        ...
                        echo "❌ Build failed. Trying to run migrations anyway..."
...
try:
    ...
    if ERROR_COUNT > 0:
        ...
        if ERROR_COUNT > len(statements) * 0.1:
            sys.exit(1)
        else:
