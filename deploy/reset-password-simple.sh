...
# Simple password reset - uses the server's own code to hash passwords correctly
...
CSPROJ
    ...
    echo "Generated password hash (first 30 chars): \${PASSWORD_HASH:0:30}..."
...
MYSQL
    ...
    if [ "\$ROWS_UPDATED" = "1" ]; then
        ...
        mysql -u mentalhealth_user -p"\$DB_PASSWORD" mentalhealthdb -e "SELECT Id, Email, FirstName, LastName FROM Users LIMIT 10;" 2>/dev/null
        exit 1
    else
