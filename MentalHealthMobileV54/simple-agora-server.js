...
// Serve the direct audio test page
app.get('/direct-audio-test.html', (req, res) => {
  res.sendFile(__dirname + '/direct-audio-test.html');
});
    ...
    <script>
        ...
        async function testAgoraConnection() {
            try {
                ...
                // Try a different approach - use live mode which often works better
                ...
                if (error.code === 4096) {
                    document.getElementById('status').innerHTML = '❌ Gateway Error 4096 - Trying WebRTC approach...';
                    
                    // Try with WebRTC mode instead
                    try {
                        ...
                        // Create audio track
                        const audioTrack = await AgoraRTC.createMicrophoneAudioTrack();
                        await client2.publish([audioTrack]);
                        
                        document.getElementById('status').innerHTML = '✅ Connected with WebRTC! Audio active!';
                        addUser('${uid}', 'You (WebRTC)');
                        return;
                        
                    } catch (error2) {
                        console.error('❌ WebRTC approach also failed:', error2);
                    }
                }
